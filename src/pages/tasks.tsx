import { Category, ProjectList, ProjectSection } from '@/misc/types'
import instance from '@/service/axios/axiosConfig'
import { Button, Chip, CircularProgress, Divider, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField } from '@material-ui/core'
...
const TaskMaker: FunctionComponent = () => {
  ...
  const handleSelect = (i: number): void => {
    if (state.list !== undefined && i !== undefined) {
      ...
      setState({ sectionName: state.list[i].name, selectedSectionId: state.list[i]._id })
    }
  }
                    ...
                    <TableBody>
                      {state.list.map((item, idx) => (
                        <TableRow onClick={() => handleSelect(idx)} key={item._id} className='cursor-pointer'>
                          <TableCell className='text-white'>{dayjs().format('YYYY/MM/DD')}</TableCell>
                ...
                {state.sections.length !== 0 ? (
                  <TableContainer className='my-4'>
                    <div className='flex justify-between items-center text-white mb-4'>
                      <span className='mx-2 text-base underline'>{state.sectionName}</span>
                      <Button className='bg-opacity-20 bg-primary text-white shadow-md hover:bg-primary' onClick={() => setState({ addModal: 'section' })}>Add Section</Button>
                    </div>
                    <Table size='small'>
                      <TableHead>
                        <TableRow>
                      ...
                      <TableBody>
                        {state.sections.map((section, i) => (
                          <TableRow key={section._id}>
                            ...
                            {section.audioPath !== null && <TableCell className='text-white'><audio controls src={`http://3.144.152.209:3002/${encodeURIComponent(section.audioPath)}`} /></TableCell>}
                  ...
                  <div className='w-36 justify-center items-center flex h-full'>
                    <Button className='bg-opacity-20 bg-primary text-white shadow-md hover:bg-primary' onClick={() => setState({ addModal: 'section', sections: undefined })}>Add Section</Button>
                  </div>
                )}
              </Paper>
            )}
            {state.sectionEdit !== null && state.sectionEdit !== undefined && (
              <Paper classes={{ root: 'bg-gray-100/25 border-none shadow-xl p-4 w-max' }}>
